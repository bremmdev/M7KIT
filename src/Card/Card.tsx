...
export const Card = <T extends React.ElementType = "div">(props: CardProps<T>) => {
  ...
  return (
    <Component className={cn("w-fit text-foreground bg-surface rounded-xl overflow-hidden", className)} {...rest}>
      {image && (
        <img src={image} alt={imageAlt} style={imageStyle} className={cn("w-full object-cover", imageClassName)} />
      )}
      <div className={cn("flex flex-col gap-4 p-8", borderClass)}>{children}</div>
    </Component>
  );
};
