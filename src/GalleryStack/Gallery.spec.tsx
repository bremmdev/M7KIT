...
describe("Gallery", () => {
  ...
  it("render correct order of items", () => {
    ...
    const items = Array.from(
      getByTestId("gallery-stack").querySelectorAll("div")
    );
  ...
  it("renders reversed order of items", () => {
    const { getByTestId } = render(
      <GalleryStack reversed>{children}</GalleryStack>
    );
  ...
  it("renders correct rotation amount", () => {
    const { getByTestId } = render(
      <GalleryStack rotationAmount={10}>{children}</GalleryStack>
    );

    const items = Array.from(
      getByTestId("gallery-stack").querySelectorAll(":scope > div")
    );
  ...
  it("renders correct negative rotation amount", () => {
    const { getByTestId } = render(
      <GalleryStack rotationAmount={-15}>{children}</GalleryStack>
    );
  ...
  it("rotates the stack forward", async () => {
    ...
    const itemsAfterClick = Array.from(
      getByTestId("gallery-stack").querySelectorAll(":scope > div")
    );
  ...
  it("rotates the stack forward on arrow right click", async () => {
    ...
    const { getByTestId, getAllByRole } = render(
      <GalleryStack>{children}</GalleryStack>
    );
